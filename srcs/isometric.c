...
void	isometric_projection(t_info *info)
{
	...
	affine_matrix = combine_affine_matrix(affine_matrix,
			affine_new_scale_matrix(info->state->scale_x, info->state->scale_y, info->state->scale_z));
	affine_matrix = combine_affine_matrix(affine_matrix,
			affine_new_translate_matrix(info->state->trans_x, info->state->trans_y, info->state->trans_z));
